/* Qualité Atelier — socle commun, parcours pack-aware (DA | S&E) */
...
(function () {
  ...
  const data = window.QUALITE_CONTENT.build(packId);
  window.ATELIER = data;
  const Lab = window.QualiteLab;

  const STORAGE_KEY = "qualite-atelier-progress-" + packId + "-v1";
  const CHECK_KEY = "qualite-atelier-checks-" + packId + "-v1";
  const BILAN_KEY = "qualite-atelier-bilan-" + packId + "-v1";
  const GOV_KEY = "qualite-atelier-gov-" + packId + "-v1";
  ...
  function saveGov() {
    localStorage.setItem(GOV_KEY, JSON.stringify(state.gov));
  }
  ...
  function syncChrome() {
    ...
    document.title =
      data.brand + (packId === "se" ? " — Qualité & pilotage" : " — Qualité des données");
  }
  ...
  function ph(label) {
    return `<div class="ph" aria-hidden="true">${escapeHtml(label || "Qualité")}</div>`;
  }
  ...
  function renderCode(code) {
    ...
    return `
      <div class="code-block">
        <header><span>${escapeHtml(code.label || "qualite")}</span><span>Qualité Atelier</span></header>
  ...
  function renderHome() {
    ...
    return `
      <section class="hero">
        <div class="hero-media">${ph(packId === "se" ? "Qualité S&E" : "Qualité DA")}</div>
        <div class="hero-content">
          ...
          <div class="hero-actions">
            ...
            <button type="button" class="btn btn-secondary" data-nav-inline="labo">Ouvrir le labo Qualité</button>
          </div>
        </div>
      </section>
      <div class="wrap">
  ...
  function renderParcours() {
    return `
      <div class="wrap">
        ...
        ${packChip()}
        <h2 class="section-title" style="font-size:1.5rem;margin-top:1.5rem">1. Socle qualité</h2>
    ...
    if (phase === "voir") {
      ...
      body = `
        ...
        <div class="phase-actions">
          ...
          <button class="btn btn-primary" data-nav-inline="labo">Labo Qualité</button>
        </div>`;
  ...
  function renderLabo() {
    const sets = Lab.datasets.filter((d) => !d.packs || d.packs.includes(packId));
    const checks = checklistItems();
    const doneChecks = checks.filter((c) => state.checks[c.id]).length;
    const doneQgis = Lab.gouvernanceSteps.filter((s) => state.gov[s.id]).length;
    return `
      <div class="wrap">
        <h1 class="section-title">Labo Qualité — Kalunga</h1>
        <p class="section-lead">Chargez les CSV, lancez le QC navigateur, puis suivez la checklist gouvernance. ${packChip()}</p>
            ...
            <label>Jeu
              <select id="dataset-select">
                ${sets
                  .map(
                    (d) =>
                      `<option value="${d.id}"${d.id === state.datasetId ? " selected" : ""}>${escapeHtml(d.title)}</option>`
                  )
                  .join("")}
              </select>
            </label>
            <button type="button" class="btn btn-dark" id="run-qc">Lancer le QC</button>
            <a class="btn btn-secondary" id="dl-csv" href="data/kalunga_nutrition_mensuel.csv" download>Télécharger CSV</a>
          </div>
          <div id="qc-hints"></div>
          <div id="qc-table" style="overflow:auto;margin:0.75rem 0"></div>
          <div id="qc-issues"></div>
        </section>

        <section class="studio-card">
          <h3>2. Checklist gouvernance</h3>
          <p class="hint">${doneQgis}/${Lab.gouvernanceSteps.length} étapes — dictionnaire, règles, journal, version, go/no-go.</p>
          ${Lab.gouvernanceSteps
            .map(
              (s) => `
            <label class="check-item">
              <input type="checkbox" data-gov="${s.id}" ${state.gov[s.id] ? "checked" : ""} />
        ...
        ${packId === "se" ? `<section class="studio-card">
          <h3>2b. Matrice indicateur × contrôles</h3>
          <ul>
            ${Lab.matriceIndicateurs.map((m) => `<li><strong>${escapeHtml(m.indicateur)}</strong> — ${escapeHtml(m.controles.join(" · "))}</li>`).join("")}
          </ul>
        </section>` : ""}
        <section class="studio-card">
          <h3>3. Checklist livrable ${packId === "se" ? "S&E" : "Data Analyst"}</h3>
  ...
  async function runQc() {
    const id = document.getElementById("dataset-select")?.value || state.datasetId;
    state.datasetId = id;
    ...
    if (!ds || !tableBox || !issuesBox) return;
    if (dl) dl.href = ds.path;
    if (hints) {
      hints.innerHTML = `<h4 style="margin:0.5rem 0">Pistes</h4><ul>${ds.hints
    ...
    try {
      ...
      tableBox.innerHTML = `
        <table class="data-table" style="width:100%;border-collapse:collapse;font-size:0.85rem">
          <thead><tr>${parsed.headers
            .map((h) => `<th style="text-align:left;border-bottom:1px solid #c5d4df;padding:0.35rem">${escapeHtml(h)}</th>`)
            .join("")}</tr></thead>
          <tbody>
            ${parsed.rows
              .slice(0, 20)
              .map(
                (r) =>
                  `<tr>${parsed.headers
                    .map(
                      (h) =>
                        `<td style="padding:0.35rem;border-bottom:1px solid #e6eef3">${escapeHtml(r[h])}</td>`
                    )
                    .join("")}</tr>`
              )
              .join("")}
        ...
        ${parsed.rows.length > 20 ? `<p class="hint">Aperçu 20 / ${parsed.rows.length} lignes.</p>` : ""}`;
      let issues = [];
      if (id === "nutrition") issues = Lab.qcNutrition(parsed.rows);
      else if (id === "wash") issues = Lab.qcWash(parsed.rows);
      else if (id === "benef") issues = Lab.qcBenef(parsed.rows);
      else issues = [];
      issuesBox.innerHTML = `
        <div class="principle" style="padding:1rem">
          <h3 style="margin-top:0">${issues.length} anomalie(s) / écart(s)</h3>
          <ul>
            ${
              issues.length
                ? issues
                    .map(
                      (x) =>
                        `<li><strong>${escapeHtml(x.type)}</strong> · ligne ${escapeHtml(String(x.row))} · ${escapeHtml(
                          x.detail
                        )}</li>`
                    )
                    .join("")
                : "<li>Aucune anomalie automatique — revérifiez quand même à la main.</li>"
            }
          </ul>
          <p class="hint">Documentez vos corrections dans le carnet avant tout reporting ou dashboard.</p>
        </div>`;
    } catch (err) {
      issuesBox.innerHTML = `<div class="feedback ko">Impossible de charger ${escapeHtml(
        ds.path
      )}. Lancez via un serveur local (python -m http.server).</div>`;
    }
  }
  ...
  function bindViewEvents() {
    ...
    if (state.view === "labo") {
      ...
      document.getElementById("dataset-select")?.addEventListener("change", (e) => {
        ...
        const ds = Lab.datasets.find((d) => d.id === state.datasetId);
        const dl = document.getElementById("dl-csv");
      ...
      main.querySelectorAll("[data-gov]").forEach((box) => {
        box.addEventListener("change", () => {
          state.gov[box.dataset.gov] = box.checked;
          saveGov();
        });
      });
        ...
        document.getElementById("bilan-result").innerHTML = `
          <div class="feedback ${passed ? "ok" : "ko"}">
            Score ${score}% (${correct}/${total}). ${
              passed
                ? "Module Qualité validé. Si vous êtes dans la plateforme, le module peut être marqué terminé automatiquement."
                : "Sous le seuil : revoyez le socle et le labo QC."
            }
          </div>
