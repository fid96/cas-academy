/* SIG Atelier — socle commun, parcours pack-aware (DA | S&E) */
...
(function () {
  ...
  const data = window.SIG_CONTENT.build(packId);
  window.ATELIER = data;
  const Lab = window.SIGLab;

  const STORAGE_KEY = "sig-atelier-progress-" + packId + "-v1";
  const CHECK_KEY = "sig-atelier-checks-" + packId + "-v1";
  const BILAN_KEY = "sig-atelier-bilan-" + packId + "-v1";
  const QGIS_KEY = "sig-atelier-qgis-" + packId + "-v1";
  ...
  function saveQgis() {
    localStorage.setItem(QGIS_KEY, JSON.stringify(state.qgis));
  }
  ...
  function syncChrome() {
    ...
    document.title =
      data.brand + (packId === "se" ? " — Pilotage spatial" : " — Analyse spatiale");
  }
  ...
  function ph(label) {
    return `<div class="ph" aria-hidden="true">${escapeHtml(label || "SIG")}</div>`;
  }
  ...
  function renderCode(code) {
    ...
    return `
      <div class="code-block">
        <header><span>${escapeHtml(code.label || "sig")}</span><span>SIG Atelier</span></header>
  ...
  function renderHome() {
    ...
    return `
      <section class="hero">
        <div class="hero-media">${ph(packId === "se" ? "SIG S&E" : "SIG DA")}</div>
        <div class="hero-content">
          ...
          <div class="hero-actions">
            ...
            <button type="button" class="btn btn-secondary" data-nav-inline="labo">Ouvrir le labo SIG</button>
          </div>
        </div>
      </section>
      <div class="wrap">
  ...
  function renderParcours() {
    return `
      <div class="wrap">
        ...
        ${packChip()}
        <h2 class="section-title" style="font-size:1.5rem;margin-top:1.5rem">1. Socle SIG</h2>
    ...
    if (phase === "voir") {
      ...
      body = `
        ...
        <div class="phase-actions">
          ...
          <button class="btn btn-primary" data-nav-inline="labo">Labo SIG</button>
        </div>`;
  ...
  function renderLabo() {
    ...
    const doneQgis = Lab.qgisSteps.filter((s) => state.qgis[s.id]).length;
    return `
      <div class="wrap">
        <h1 class="section-title">Labo SIG — Kalunga</h1>
        <p class="section-lead">Chargez les CSV, lancez le QC navigateur, puis suivez la checklist QGIS hors ligne. ${packChip()}</p>
        ...
        <section class="studio-card">
          ...
          <div class="lab-selects">
            ...
            <a class="btn btn-secondary" id="dl-csv" href="data/kalunga_sites_gps.csv" download>Télécharger CSV</a>
          </div>
          <div id="qc-hints"></div>
        ...
        <section class="studio-card">
          <h3>2. Checklist QGIS (hors navigateur)</h3>
          <p class="hint">${doneQgis}/${Lab.qgisSteps.length} étapes — faites-les dans QGIS avec les CSV téléchargés.</p>
          ${Lab.qgisSteps
            .map(
              (s) => `
            <label class="check-item">
              <input type="checkbox" data-qgis="${s.id}" ${state.qgis[s.id] ? "checked" : ""} />
  ...
  async function runQc() {
    ...
    try {
      ...
      let issues = [];
      if (id === "gps") issues = Lab.qcGps(parsed.rows);
      else if (id === "aires") issues = Lab.qcAires(parsed.rows);
      else issues = Lab.qcCouverture(parsed.rows);
      issuesBox.innerHTML = `
        <div class="principle" style="padding:1rem">
          ...
          <p class="hint">Documentez vos corrections dans le carnet avant toute carte officielle.</p>
        </div>`;
    } catch (err) {
  ...
  function bindViewEvents() {
    ...
    if (state.view === "labo") {
      ...
      main.querySelectorAll("[data-qgis]").forEach((box) => {
        box.addEventListener("change", () => {
          state.qgis[box.dataset.qgis] = box.checked;
          saveQgis();
        });
      });
        ...
        document.getElementById("bilan-result").innerHTML = `
          <div class="feedback ${passed ? "ok" : "ko"}">
            Score ${score}% (${correct}/${total}). ${
              passed
                ? "Module SIG validé. Si vous êtes dans la plateforme, le module peut être marqué terminé automatiquement."
